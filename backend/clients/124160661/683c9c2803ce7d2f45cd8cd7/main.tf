terraform {
  backend "http" {
    address        = "https://tidy-definitely-sailfish.ngrok-free.app/terraform/state/683c9c2803ce7d2f45cd8cd7"
    lock_address   = "https://tidy-definitely-sailfish.ngrok-free.app/terraform/lock/683c9c2803ce7d2f45cd8cd7"
    unlock_address = "https://tidy-definitely-sailfish.ngrok-free.app/terraform/unlock/683c9c2803ce7d2f45cd8cd7"
    lock_method    = "POST"
    unlock_method  = "POST"
  }
}



provider "aws" {
region = "us-east-1"
}

# Create a new key pair using the provided public key
resource "aws_key_pair" "instance_key" {
  key_name   = "683c9c2803ce7d2f45cd8cd7-key"  # Key name will match instance name
  public_key = "ssh-rsa  omar@DESKTOP-C4RISD1.pub"         # Use the local public key
}

resource "aws_instance" "web" {
    ami = "ami-0a7d80731ae1b2435"
    instance_type = "t2.micro"
    tags = {
        Name = "683c9c2803ce7d2f45cd8cd7"
    }  
    key_name = aws_key_pair.instance_key.key_name 

    vpc_security_group_ids = [aws_security_group.web_sg.id]
}


resource "aws_security_group" "web_sg" {
  name = "683c9c2803ce7d2f45cd8cd7-sg"

  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  ingress {
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
  }
}

output "instance_public_ip" {
  value       = aws_instance.web.public_ip
  description = "The public IP of the EC2 instance"
}